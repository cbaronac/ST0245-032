...
public class DigraphAlgorithms
{
    ...
    /*
     * Metodo que permite decir que vertice posee más sucesores
     * @param Grafo g y un Vertice tipo Int
     */
    public static int maxSucesores(Graph g, int vertice) {
        int temporal;
        ArrayList<Integer> lista=g.getSuccessors(vertice);
        int max=g.getSuccessors(lista.get(0)).size();
        
        for (int i=0;i<g.size();i++){
           temporal=g.getSuccessors(i).size();
           if (temporal>max){
               max=temporal;
            }
        }
